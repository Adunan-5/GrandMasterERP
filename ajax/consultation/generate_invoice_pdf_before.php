...
if ($invoiceID === null || $invoiceID === false || filter_var($invoiceID, FILTER_VALIDATE_INT) === false) {
    header("location:/consultation/invoice/list");
    exit();
}
...
//$this->SetXY(5, 30);
        ...
        $this->Cell(0, $lineHeight, $arabicLabel, 0, 0, 'R');
        $this->setX($pageWidth - $arabicLabelWidth - $pageMargin - 2);
        $this->Cell(1, $lineHeight,  convertToArabicNumbers(formatHijriDate($invoiceDocument->invoiceDateIssued, false)), 0, 0, 'R');
...
$invoiceTotal = number_format(getTotalAmountByConsultationInvoiceID($invoiceID)['totalAmountAfterVAT'], 2, '.', ''); // Total after VAT
$invoiceTax   = number_format(getTotalAmountByConsultationInvoiceID($invoiceID)['totalVATAmount'], 2, '.', '');      // VAT amount
...
//    $imageFullPath = $imageBasePath . $imageUrl;
    $itemName             = getItemInfoForConsultationServiceItemID($rowLineItem['itemId'])['itemName'];
    $description            = getItemInfoForConsultationServiceItemID($rowLineItem['itemId'])['description'];
    ...
    $item    = [
        ...
        ['itemName' => $itemName, 'description' => $description], // Part Number & Description as an array
...
foreach ($items as $item) {
    ...
    foreach ($item as $key => $value) {
        ...
        if ($key == 1 && !empty($value)) { // Image column
            ...
            $pdf->SetFont($fontName, 'B', 10);
            $partHeight = $pdf->getStringHeight($cellWidth, $itemName);
...
$pdf->Cell(95, 10, 'Total Amount (Before VAT & Discount): ₥ ' . getTotalAmountByConsultationInvoiceID($invoiceID)['totalAmountBeforeVAT'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());    // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, 'المبلغ الاجمالي (قبل الضريبة و الخصم):  ' . convertToArabicNumbers(getTotalAmountByConsultationInvoiceID($invoiceID)['totalAmountBeforeVAT']) .' ₥ ', 0, 0, 'R');

// Add VAT amount details (English and Arabic on the same line)
$pdf->SetXY(10, $pdf->GetY() + 5); // Move down to the next line
$pdf->Cell(95, 10, 'VAT Amount: ₥ ' . getTotalAmountByConsultationInvoiceID($invoiceID)['totalVATAmount'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());    // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, 'قيمة الضريبة:  ' . convertToArabicNumbers(getTotalAmountByConsultationInvoiceID($invoiceID)['totalVATAmount']).' ₥ ', 0, 0, 'R');


// Add Total discount (English and Arabic on the same line)
$pdf->SetXY(10, $pdf->GetY() + 5); // Move down to the next line
$pdf->Cell(95, 10, 'Total Discount: ₥ ' . getTotalAmountByConsultationInvoiceID($invoiceID)['totalDiscountAmount'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());    // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, ' اجمالي مبلغ الخصم:  ' . convertToArabicNumbers(getTotalAmountByConsultationInvoiceID($invoiceID)['totalDiscountAmount']) .' ₥ ', 0, 0, 'R');

// Add total amount after VAT details (English and Arabic on the same line)
$pdf->SetXY(10, $pdf->GetY() + 5); // Move down to the next line
$pdf->Cell(95, 10, 'Total Amount (After VAT & Discount): ₥ ' . getTotalAmountByConsultationInvoiceID($invoiceID)['totalAmountAfterVAT'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());     // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, '  المبلغ الاجمالي (بعد الضريبة و الخصم): ' . convertToArabicNumbers(getTotalAmountByConsultationInvoiceID($invoiceID)['totalAmountAfterVAT']) .' ₥ ', 0, 0, 'R');
