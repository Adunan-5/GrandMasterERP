...
try {
    ...
    $baseQuery = "SELECT
                    ...
                    FROM
                      `consultation_purchase_orders`
                      LEFT JOIN `suppliers` ON `consultation_purchase_orders`.`supplierId` =
                    `suppliers`.`supplierId`
                      LEFT JOIN `payment_terms` ON `consultation_purchase_orders`.`paymentTermId` =
                    `payment_terms`.`termId`
                      LEFT JOIN `users` ON `consultation_purchase_orders`.`salesPersonId` = `users`.`userID`
                  WHERE status = 'Active'";
