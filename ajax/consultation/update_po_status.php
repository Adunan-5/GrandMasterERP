...
try {
    if (!empty($documentId)) {
        ...
        if ($status == PO_STATUS_AWAITING_SPAREPARTS_MANAGER_APPROVAL) {
            $res = $db->query("UPDATE `consultation_purchase_orders` SET `poStatus`= ?s WHERE `poId`=?s", $status, $documentId);
            recordConsultationPOHistory($documentId, $status,logActivity($status) );
            echo "SUCCESS|Sent for Approval";
            exit();
        }

        if ($status == PO_STATUS_SPAREPARTS_MANAGER_APPROVED) {
            $res = $db->query("UPDATE `consultation_purchase_orders` SET `poStatus`= ?s, `spManagerPOApproved` = 'APPROVED' WHERE `poId`=?s", $status, $documentId);
            recordConsultationPOHistory($documentId, $status,logActivity($status) );
        ...
        if ($status == PO_STATUS_ACCOUNTANT_APPROVED) {
            ...
            $poDATA         = "";
            $res            = $db->query("SELECT * FROM consultation_purchase_orders WHERE active = 1 and poId = ?s", $documentId);
            ...
            $poNumberPrefix = PREFIX_PO_CONSULTATION;
            if (empty($poDATA['poNumber'])) {
                $poNumber = getNextNewConsultationPONumber();
            } else {
                $poNumber = $poDATA['poNumber'];
            }
            $res = $db->query("UPDATE `consultation_purchase_orders` SET `poStatus`= ?s, `poNumber`=?s, `poNumberPrefix`=?s, `accountantPOApproved` = 'APPROVED' WHERE `poId`=?s", $status, $poNumber, $poNumberPrefix, $documentId);
            recordConsultationPOHistory($documentId, $status,logActivity($status), "PO Number generated - " . getPONumberFromConsultationDocumentID($documentId) );
        ...
        if ($status == PO_STATUS_SPAREPARTS_MANAGER_REJECTED) {
            $res = $db->query("UPDATE `consultation_purchase_orders` SET `poStatus`= ?s WHERE `poId`=?s", $status, $documentId);
            $res = $db->query("INSERT INTO `consultation_po_reject_reason_sp_manager` (`poId`, `rejectReason`, `rejectedBy`) VALUES (?s, ?s, ?s)", $documentId, $rejectReason, getAuthenticatedUser()->userID);
            recordConsultationPOHistory($documentId, $status,logActivity($status), $rejectReason );
        ...
        if ($status == PO_STATUS_ACCOUNTANT_REJECTED) {
            $res = $db->query("UPDATE `consultation_purchase_orders` SET `poStatus`= ?s WHERE `poId`=?s", $status, $documentId);
            $res = $db->query("INSERT INTO `consultation_po_reject_reason_accountant` (`poId`, `rejectReason`, `rejectedBy`) VALUES (?s, ?s, ?s)", $documentId, $rejectReason, getAuthenticatedUser()->userID);
            recordConsultationPOHistory($documentId, $status,logActivity($status), $rejectReason );
            echo "SUCCESS|Purchase Order has been Rejected";
            exit();
        }
