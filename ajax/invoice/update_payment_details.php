...
if ($data) {
    ...
    try {
        if (!empty($accountsReceivableID)) {
            ...
            $res = $db->query("INSERT INTO `journal_entries` (`entryDate`, `description`, `referenceId`,`referenceType`, `companyId`) VALUES (now(), ?s, ?s, ?s, ?s)", "Customer Payment for the Invoice #" . getInvoiceNumberFromInvoiceID($invoiceID), $paymentId, "CUSTOMERPAYMENT", $companyId);
            $journalEntryId = $db->insertId();
            $res = $db->query("INSERT INTO `journal_entry_lines` (`journalEntryId`, `accountId`, `debit`, `credit`, `description`) VALUES (?s, ?s, ?s, ?s, ?s)", $journalEntryId, $cashOrBankAccountID, $amountReceived, 0, "Payment Received from Customer for the Invoice #" . getInvoiceNumberFromInvoiceID($invoiceID));
            $res = $db->query("INSERT INTO `journal_entry_lines` (`journalEntryId`, `accountId`, `debit`, `credit`, `description`) VALUES (?s, ?s, ?s, ?s, ?s)", $journalEntryId, $arAccountID, 0, $amountReceived, "Reducing the Accounts Receivable for the Invoice #" . getInvoiceNumberFromInvoiceID($invoiceID));
