...
//$this->SetXY(5, 30);
        ...
        $this->Cell(0, $lineHeight, 'Invoice #: ' . $quotationNumber, 0, 0, 'L');
        ...
        $this->Cell(1, $lineHeight, convertToArabicNumbers($quotationNumber), 0, 1, 'R');
        ...
        $this->Cell(0, $lineHeight, $arabicLabel, 0, 0, 'R');
        $this->setX($pageWidth - $arabicLabelWidth - $pageMargin - 2);
        $this->Cell(1, $lineHeight,  convertToArabicNumbers(convertToHijri($keyDocument->quotationDateIssued)), 0, 0, 'R');
...
if ($documentID == "32")
    $quotationNumber = "GM-INV-000026";
else if ($documentID == "35")
    $quotationNumber = "GM-INV-000027";
else
    $quotationNumber = "GM-INV-000028";

$keyDocument->quotationDateIssued = date("Y-m-d H:i:s");
...
$pdf->SetTitle('Invoice ' . $quotationNumber);
...
$sellerName = $companyInfo->companyName;                // Grandmaster company name
...
$invoiceDate  = (new DateTime($keyDocument->quotationDateIssued, new DateTimeZone('Asia/Riyadh')))
    ->setTimezone(new DateTimeZone('UTC'))
    ->format('Y-m-d\TH:i:s\Z');                                                                                         // Convert to Zulu format
$invoiceTotal = number_format(getTotalAmountByDocumentID($keyDocument->documentId)['totalAmountAfterVAT'], 2, '.', ''); // Total after VAT
$invoiceTax   = number_format(getTotalAmountByDocumentID($keyDocument->documentId)['totalVATAmount'], 2, '.', '');      // VAT amount
...
$details = [
    ...
    ['Cust. PO# ', $keyDocument->PONumber, 'امر شراء العميل       ', convertToArabicNumbers($keyDocument->PONumber)],
    ...
    ['Payment Terms ', $keyDocument->getPaymentTermName(), 'شروط الدفع       ', convertToArabicNumbers($keyDocument->getPaymentTermName())]
];
...
$pdf->Cell(95, 10, 'Total Amount (Before VAT & Discount): ₥ ' . getTotalAmountByDocumentID($documentID)['totalAmountBeforeVAT'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());    // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, 'المبلغ الاجمالي (قبل الضريبة و الخصم):  ' . convertToArabicNumbers(getTotalAmountByDocumentID($documentID)['totalAmountBeforeVAT']) .' ₥ ', 0, 0, 'R');

// Add VAT amount details (English and Arabic on the same line)
$pdf->SetXY(10, $pdf->GetY() + 5); // Move down to the next line
$pdf->Cell(95, 10, 'VAT Amount: ₥ ' . getTotalAmountByDocumentID($documentID)['totalVATAmount'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());    // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, 'قيمة الضريبة:  ' . convertToArabicNumbers(getTotalAmountByDocumentID($documentID)['totalVATAmount']).' ₥ ', 0, 0, 'R');


// Add Total discount (English and Arabic on the same line)
$pdf->SetXY(10, $pdf->GetY() + 5); // Move down to the next line
$pdf->Cell(95, 10, 'Total Discount: ₥ ' . getTotalAmountByDocumentID($documentID)['totalDiscountAmount'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());    // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, ' اجمالي مبلغ الخصم:  ' . convertToArabicNumbers(getTotalAmountByDocumentID($documentID)['totalDiscountAmount']) .' ₥ ', 0, 0, 'R');

// Add total amount after VAT details (English and Arabic on the same line)
$pdf->SetXY(10, $pdf->GetY() + 5); // Move down to the next line
$pdf->Cell(95, 10, 'Total Amount (After VAT & Discount): ₥ ' . getTotalAmountByDocumentID($documentID)['totalAmountAfterVAT'], 0, 0, 'L');
$pdf->SetXY(190, $pdf->GetY());     // Adjust X-coordinate for right alignment and keep the Y-coordinate the same
$pdf->Cell(95, 10, '  المبلغ الاجمالي (بعد الضريبة و الخصم): ' . convertToArabicNumbers(getTotalAmountByDocumentID($documentID)['totalAmountAfterVAT']) .' ₥ ', 0, 0, 'R');
...
if ($isDownload == 1)
// Output the PDF
    $pdf->Output('Invoice_' . $quotationNumber . '.pdf', 'D');
else
    $pdf->Output('Invoice_' . $quotationNumber . '.pdf', 'I');
