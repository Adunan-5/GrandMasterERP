...
// Title
        ...
        $titleText   = 'Goods Delivery Note ' . 'سند تسليم بضاعة';
...
while ($rowLineItem = mysqli_fetch_assoc($resLineItems)) {
    ...
    $resOrderLineItem = $db->query("SELECT quantity FROM order_management_line_items WHERE `itemId` = ?s AND `orderManagementId` = ?s LIMIT 1", $rowLineItem['itemId'], $rowLineItem['orderManagementId']);
    $orderLineItem = mysqli_fetch_assoc($resOrderLineItem);
    $requestedQuantity = $orderLineItem['quantity'];
