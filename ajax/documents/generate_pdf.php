...
$quotationHeaders = [
    ...
    ['تاريخ عرض السعر', 'Quotation Date'],
    ['شروط الدفع', 'Payment Terms'],
    ['تاريخ انتهاء عرض السعر', 'Quotation Expiry'],
...
while ($rowLineItem = mysqli_fetch_assoc($resLineItems)) {
    ...
    $partNumber = getDetailsForSparepartID($rowLineItem['itemId']);
    ...
    $itemType = getItemTypeForSparepartID($rowLineItem['itemId']);
    ...
    $item    = [
        ...
        ['partNumber' => $partNumber, 'description' => $description,  'brand' => $brandName, 'itemType' => $itemType], // Part Number & Description as an array
...
foreach ($items as $item) {
    ...
    foreach ($item as $key => $value) {
        ...
        if ($key == 1) { // Image column
            $itemType = isset($item[2]) && is_array($item[2]) ? $item[2]['itemType'] ?? '' : '';
            $showImage = ($itemType !== 'SERVICE') && !empty($value);
            $rowHeights[] = $showImage ? 30 : $defaultHeight;
            ...
            $manufacturerHeight = 0;
            $showManufacturer = ($itemType !== 'SERVICE');
            if ($showManufacturer) {
                $manufacturerLine = 'Manufacturer: ' . $brandName;
                $manufacturerHeight = $pdf->getStringHeight($cellWidth, $manufacturerLine);
            }

            $rowHeights[] = $partHeight + $descHeight + ($showManufacturer ? $manufacturerHeight + 10 : 10);
        } else {
            $pdf->SetFont($fontName, '', 10);
    ...
    foreach ($item as $key => $value) {
        ...
        if ($key == 1) { // Image column
            ...
            $contentHeight = $partHeight + $descHeight + ($showManufacturer ? max($manufacturerHeight, 5) : 0);
