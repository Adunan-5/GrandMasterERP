...
<head>
  ...
  <title>GrandMaster ERP | Consultation POs</title>
            ...
            <div class="card-widget-separator-wrapper">
              <div class="card-body card-widget-separator py-2">
                <div class="row gy-1 gy-sm-1">
                  <h4 class="my-0">All Consultation POs</h4>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="card-datatable table-responsive">
              <table class="invoice-list-table table border-top">
                ...
                <tr>
                  <th></th>
                  <th></th>
                  <th class="text-truncate">PO #</th>
                  <th class="cell-fit">Supplier</th>
                  <th>Payment Term</th>
                  <th>RFP #</th>
                  <th>Reason</th>
                  <th>Status</th>
...
<script>
  ...
  $(function () {
    ...
    if (dt_invoice_table.length) {
      ...
      let ajaxURL = '/ajax/consultation/fetch_pos.php';
      ...
      var dt_invoice = dt_invoice_table.DataTable({
        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              ...
              return (
                '<div class="d-flex align-items-center">' +
                // '<a href="javascript:;" data-bs-toggle="tooltip" class="btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Delete"><i class="ti ti-trash mx-2 ti-md"></i></a>' +
                // '<a href="/purchaseorder/edit/' + $poId + '" data-bs-toggle="tooltip" class="btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title=""><i class="ti ti-eye mx-2 ti-md"></i></a>' +
                '<div class="dropdown">' +
                '<a href="javascript:;" class="btn dropdown-toggle hide-arrow btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill p-0" data-bs-toggle="dropdown"><i class="ti ti-dots-vertical ti-md"></i></a>' +
                '<div class="dropdown-menu dropdown-menu-end">' +
                '<a href="/ajax/podocument/generate_pdf.php?isdownload=1&poID=' + $poId + '" class="dropdown-item" target="_blank">Download</a>' +
                '<a href="/purchaseorder/edit/' + $poId + '" class="dropdown-item">Edit</a>' +
                '</div>' +
                '</div>'
              );
            }
...

          {
            ...
            targets: 2,
            render: function (data, type, full, meta) {
              var $poNumber = full['poNumber'];
              var $poId = full['poId'];
              var $poDateCreated = full['poDateCreated'];

              // Creates full output for row
              // if ($quotationNumber === null) $quotationNumber = "Draft";
              var $row_output = '<a class="text-truncate" href="/purchaseorder/edit/' + $poId + '"># ' + $poNumber + '</a>';
...

          {
            ...
            responsivePriority: 2,
            render: function (data, type, full, meta) {
              var $rfpNo = full['rfpId'];

              return '<span class=text-truncate>' + $rfpNo + '</span>';
            }
          },
          {
            // Status
            ...
            render: function (data, type, full, meta) {
              ...
              } else if ($status === "AWAITING SPAREPARTS MANAGER APPROVAL" || $status === "AWAITING ACCOUNTANT APPROVAL") {
                $badge_class = 'bg-label-warning';
                return ('<span class="badge ' + $badge_class + '" text-capitalized>' + $status + '</span>');
              } else if ($status === "SPAREPARTS MANAGER APPROVED" || $status === "ACCOUNTANT APPROVED" || $status === "GOODS RECEIVED") {
                $badge_class = 'bg-label-success';
                return ('<span class="badge ' + $badge_class + '" text-capitalized>' + $status + '</span>');
              } else if ($status === "SPAREPARTS MANAGER REJECTED" || $status === "ACCOUNTANT REJECTED") {
                $badge_class = 'bg-label-danger';
                return ('<span class="badge ' + $badge_class + '" text-capitalized>' + $status + '</span>');
              } else if ($status === "PO SENT TO SUPPLIER") {
          ...
          },
          {
            ...
            targets: 8,
            render: function (data, type, full, meta) {
              var $poDate = new Date(full['poDateCreated']);

              let $row_output;
              if (full['poDateCreated'] != null) {
                $row_output = '<span class="d-none">' + moment($poDate).format('YYYYMMDD') + '</span><span class="text-truncate">' + moment($poDate).format('DD MMM YYYY') + '</span>';
              } else {
                $row_output = "-";
              }
        ...
        responsive: {
          details: {
            display: $.fn.dataTable.Responsive.display.modal({
              header: function (row) {
                var data = row.data();
                return 'Details of ' + data['poNumber'];
              }
            }),
            type: 'column',
