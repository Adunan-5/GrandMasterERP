                    ...
                    <div class="card">
                        <div class="card-datatable table-responsive">
                            <table class="invoice-list-table table border-top">
                                ...
                                <tbody>
                                <?php
                                $res = $db->query("SELECT sp.*, sc.categoryName 
                                                    FROM spareparts sp
                                                    LEFT JOIN services_category sc ON sp.categoryId = sc.categoryId
                                                    WHERE sp.itemType = 'SERVICE'");

                                while ($row = mysqli_fetch_assoc($res)) {
                                    $uomName = getUOMNameFromID($row['uomId']) ?: 'N/A';
                                    ?>
                                    <tr>
                                        <td></td>
                                        <td></td>
                                        <td><?= $row['sparepartId'] ?></td>
                                        <td><?= htmlspecialchars($row['name'], ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?= nl2br(htmlspecialchars($row['description'] ?? '', ENT_QUOTES, 'UTF-8', false)) ?></td>
                                        <td><?= $uomName ?></td>
                                        <td><?= htmlspecialchars($row['categoryName'] ?? 'N/A', ENT_QUOTES, 'UTF-8') ?></td>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <a href="javascript:;" 
                                                data-servicename="<?= htmlspecialchars($row['name'], ENT_QUOTES, 'UTF-8') ?>" 
                                                data-servicedescription="<?= htmlspecialchars($row['description'] ?? '', ENT_QUOTES, 'UTF-8') ?>" 
                                                data-serviceid="<?= $row['sparepartId'] ?>" 
                                                data-serviceuomid="<?= $row['uomId'] ?>"
                                                data-serviceuom="<?= $uomName ?>" 
                                                data-servicecategoryid="<?= $row['categoryId'] ?>" 
                                                data-serviceprice="<?= $row['salesPrice'] ?>" 
                                                data-servicecost="<?= $row['cost'] ?>" 
                                                data-servicestatus="<?= $row['active'] ?>" 
                                                data-bs-toggle="offcanvas" 
                                                data-bs-target="#offcanvasServiceEdit" 
                                                class="btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill serviceEditButton" 
                        ...
                        <form class="ecommerce-customer-add pt-0" id="serviceAddForm" method="POST" name="serviceAddForm" onsubmit="return false;">
                            <div class="ecommerce-customer-add-basic mb-4">
                                ...
                                <div class="mb-6">
                                    ...
                                    <select class="form-select w-100" id="serviceUOM" name="serviceUOM" data-style="btn-default" required>
                                        <option value="">Select UOM</option>
                                        <?php
                                        $res = $db->query("SELECT * FROM uoms WHERE active = 1");
                                        while ($row = mysqli_fetch_assoc($res)) {
                                            ?>
                                            <option value="<?= $row['uomId'] ?>"><?= htmlspecialchars($row['uomName'], ENT_QUOTES, 'UTF-8') ?></option>
                                            <?php
                                        }
                                        ?>
                                    </select>
                                ...
                                <div class="mb-6">
                                    ...
                                    <select class="form-select selectpicker w-100" id="serviceCategory" name="serviceCategory" data-style="btn-default" data-live-search="true" required>
                                        ...
                                        <?php
                                        $res = $db->query("SELECT * FROM services_category");
                                        while ($row = mysqli_fetch_assoc($res)) {
                                            ?>
                    ...
                    <div class="offcanvas-body border-top mx-0 flex-grow-0">
                        <form class="ecommerce-customer-add pt-0" id="serviceEditForm" method="POST" name="serviceEditForm" onsubmit="return false;">
                            <div class="ecommerce-customer-add-basic mb-4">
                                ...
                                <div class="mb-6">
                                    <label class="form-label" for="serviceUOM">UOM*</label>
                                    <select class="form-select w-100" id="editServiceUOM" name="serviceUOM" data-style="btn-default" required>
                                        <option value="">Select UOM</option>
                                        <?php
                                        $res = $db->query("SELECT * FROM uoms WHERE active = 1");
                                        while ($row = mysqli_fetch_assoc($res)) {
                                ...
                                <div class="mb-6">
                                    ...
                                    <select class="form-select w-100" id="editServiceCategory" name="serviceCategory" data-style="btn-default" required>
                                        <option value="">Select Category</option>
                                        <?php
                                        $res = $db->query("SELECT * FROM services_category WHERE active = 1");
                                        while ($row = mysqli_fetch_assoc($res)) {
                                            ?>
                                            <option value="<?=$row['categoryId'] ?>"><?= $row['categoryName'] ?></option>
                                            <?php
...
<script>
        ...
        $("#serviceAddForm").submit(function (e) {
            ...
            fv.validate().then(function (status) {
                if (status === 'Valid') {
                    ...
                    $.ajax({
                        url: '/ajax/services/add_service.php',
                        type: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
        ...
        $("#serviceEditForm").submit(function (e) {
            ...
            editFormValidation.validate().then(function (status) {
                if (status === 'Valid') {
                    ...
                    $.ajax({
                        url: '/ajax/services/edit_service.php',
                        type: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
    ...
    $(document).on('click', '.serviceEditButton', function (e) {
        ...
        let serviceUOMId = $(this).data("serviceuomid");
        ...
        $("#editServiceUOM").val(serviceUOMId);
