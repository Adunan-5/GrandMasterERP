...
<script>
    ...
    function applySelectPicker() {
        ...
        const sparepartitem = $('[name="line-item[0][sparepartitem]"]');
        sparepartitem.selectpicker();
    }
    ...
    function applyAutoComplete() {
        //Spareparts
        // $(".sparepartitem").autocomplete({
        //     source: function (request, response) {
        //         $.ajax({
        //             url: '/ajax/spareparts/get_spareparts.php',
        //             method: 'GET',
        //             data: {term: request.term}, // Pass the entered value to the server
        //             dataType: 'json',
        //             success: function (data) {
        //                 sparepartsAutoCompleteData = data;
        //                 response(data);
        //             }
        //         });
        //     },
        ...
        //
        //         // Store the selected ID in a hidden input
        //         const index = getRepeaterRowIndex($(this));
        //
        //
        //         const hiddenInputName = $(this).attr("name").replace("[sparepartitem]", "[sparepartid]");
        //         const hiddenInput = $(`<input type="hidden" name="${hiddenInputName}" value="${ui.item.value}">`);
        //         $(this).next('input[type="hidden"]').remove(); // Remove any existing hidden input
        //         $(this).after(hiddenInput);
        //
        //         const sparepartdescription = getElementByIndexAndName(index, 'sparepartdescription');
        //         sparepartdescription.val(ui.item.desc);
        //
        //         const qty = getElementByIndexAndName(index, 'qty');
        //         qty.val('1');
        //
        //         const unitPrice = getElementByIndexAndName(index, 'unitPrice');
        //         unitPrice.val(ui.item.salesPrice);
        //
        ...
        //         hsPercentage.val(ui.item.hsPercentage);
        //
        ...
        //         return false; // Prevent the default behavior of setting value
        //     },
        //     response: function () {
        //         // Reset the selected flag when new suggestions are fetched
        ...
        //         source: function (request, response) {
        //             $.ajax({
        //                 url: '/ajax/spareparts/get_uoms.php',
        //                 method: 'GET',
        //                 data: {term: request.term}, // Pass the entered value to the server
        //                 dataType: 'json',
        //                 success: function (data) {
        //                     // Return the response as an array of { label, value }
        //                     response(data);
        //                 }
        //             });
        //         },
        //         select: function (event, ui) {
        //             // Store the selected ID in a hidden input
        //             const hiddenInputName = $(this).attr("name").replace("[uom]", "[uomid]");
        //             const hiddenInput = $(`<input type="hidden" name="${hiddenInputName}" value="${ui.item.value}">`);
        //             $(this).next('input[type="hidden"]').remove(); // Remove any existing hidden input
        //             $(this).after(hiddenInput);
        //
    ...
    $(document).on('change', '.sparepartitem', function (e) {
        ...
        // <option value="1" data-desc="CONTROL BOARD" data-leadtime="0.00" data-salesprice="25.00" data-hscode="0" data-hspercentage="0.00" data-uom="Each">GM-006780</option>
        const selectedDesc = $(this).find(':selected').data('desc');
        const selectedUnitPrice = $(this).find(':selected').data('salesprice');
        const selectedhsCode = $(this).find(':selected').data('hscode');
        const selectedhsPercentage = $(this).find(':selected').data('hspercentage');
        const selectedUOMID = $(this).find(':selected').data('uomid');
        const selectedleadtime = $(this).find(':selected').data('leadtime');
