...
<head>
    ...
    <title>GrandMaster ERP | SaleOrders</title>
                        ...
                        <div class="card-widget-separator-wrapper">
                            <div class="card-body card-widget-separator py-2">
                                <div class="row gy-1 gy-sm-1">
                                    <h4 class="my-0">All Sales Order</h4>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-datatable table-responsive">
                            <table class="invoice-list-table table border-top">
                                ...
                                <tr>
                                    <th></th>
                                    <th></th>
                                    <th class="text-truncate">Sales Order #</th>
                                    <th class="cell-fit">Customer</th>
                                    <th>Payment Term</th>
                                    <th>Status</th>
                                    <th class="text-truncate">SO Date</th>
    ...
    $(function () {
        ...
        if (dt_invoice_table.length) {
            var dt_invoice = dt_invoice_table.DataTable({
                ajax: '/ajax/documents/fetch_salesorders.php',
                columns: [
                    ...
                    {data: 'companyName', orderData: 'customerId'},
                    {data: 'termName'},
                    {data: 'saleOrderStatus'},
                    {data: 'saleOrderDateIssued'},
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        targets: 2,
                        render: function (data, type, full, meta) {
                            var $salesOrderNumber = full['saleOrderNumberPrefix'] + full['saleOrderNumber'];
                            var $documentId = full['documentId'];
                            var $salesOrderDate = full['saleOrderDateIssued'];

                            var $row_output = '<a class="text-truncate" href="/salesorders/edit/' + $documentId + '"># ' + $salesOrderNumber + '</a>' +
                                 '<br><small class="text-truncate">' + full['quotationNumber'] + '</small>';
                            return $row_output;
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        targets: 5,
                        render: function (data, type, full, meta) {
                            var $status = full['saleOrderStatus'];
                            let $badge_class = '';
                            //'ACTIVE','CONFIRMED','ON HOLD','PICK AND PACK','CLOSED','CANCELLED'


                            if ($status === "ACTIVE") {
                                $badge_class = 'bg-label-linkedin';
                                return ('<span class="badge ' + $badge_class + '" text-capitalized> ACTIVE </span>');
                            ...
                            return ('<span class="text-capitalized badge bg-label-primary">' + $status + '</span>');
                        }
                    },
                    {
                        // SO Date
                        targets: 6,
                        render: function (data, type, full, meta) {
                            var $saleOrderDate = new Date(full['saleOrderDateIssued']);

                            let $row_output;
                            if (full['saleOrderDateIssued'] != null) {
                                $row_output = '<span class="d-none">' + moment($saleOrderDate).format('YYYYMMDD') + '</span><span class="text-truncate">' + moment($saleOrderDate).format('DD MMM YYYY') + '</span>';
                            } else {
                                $row_output = "-";
                            }
