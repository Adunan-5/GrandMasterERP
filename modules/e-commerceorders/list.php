                        ...
                        <div class="card-widget-separator-wrapper">
                            <div class="card-body card-widget-separator py-2">
                                <div class="row gy-1 gy-sm-1">
                                    <h4 class="my-0">All E-commerce Orders</h4>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-datatable table-responsive">
                            <table class="invoice-list-table table border-top">
                                ...
                                <tr>
                                    ...
                                    <th>Payment Term</th>
                                    <th>Status</th>
                                    <th class="text-truncate">E-Commerce Date</th>
                                    <th>SO#</th>
                                    <th>Amount</th>
    ...
    $(function () {
        ...
        if (dt_invoice_table.length) {
            var dt_invoice = dt_invoice_table.DataTable({
                ajax: '/ajax/documents/fetch_ecommerceorders.php',
                columns: [
                ...
                columnDefs: [
                        ...
                        render: function (data, type, full, meta) {
                          var $documentId = full['documentId'];
                            return (
                                ...
                                '<a href="/ecommerce/edit/' + $documentId + '" class="dropdown-item">Edit</a>' +
                                '</div>' +
                                '</div>'
                            );
                        }
                    },
                    {
                        ...
                        targets: 2,
                        render: function (data, type, full, meta) {
                            var $orderNumber = full['orderId'];
                            var $documentId = full['documentId'];

                            var $row_output = '<a class="text-truncate" href="/ecommerce/edit/' + $documentId + '"># ' + $orderNumber + '</a>' +
                                 '<br>';
                            return $row_output;
                        }
                    },
                    ...
                    },
                    {
                      ...
                      targets: 6,
                      render: function (data, type, full, meta) {
                        var $orderDate = new Date(full['orderDateIssued']);

                        let $row_output;
                        if (full['orderDateIssued'] != null) {
                          $row_output = '<span class="d-none">' + moment($orderDate).format('YYYYMMDD') + '</span><span class="text-truncate">' + moment($orderDate).format('DD MMM YYYY') + '</span>';
                        } else {
                          $row_output = "-";
                        }
                    ...
                    },
                  {
                    ...
                    targets: 7,
                    render: function (data, type, full, meta) {
                      var $salesorderNumber = full['salesorderNumber'];
                      var $documentId = full['documentId'];

                      // Check if salesorderNumber is '-' or empty/null
                      if ($salesorderNumber === '-' || !$salesorderNumber) {
                        return $salesorderNumber || '-'; // Return '-' if empty/null, otherwise return the value
                      } else {
                        // Create hyperlink only if salesorderNumber is valid
                        return '<a class="text-truncate" href="/salesorders/edit/' + $documentId + '" target="_blank" rel="noopener noreferrer"># ' + $salesorderNumber + '</a>';
                      }
                    }
                  },
                ],
