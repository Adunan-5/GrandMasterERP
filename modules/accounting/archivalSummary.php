<?php
$PAGE_ID = "ACCOUNTS_INVOICE_ARCHIVAL_SUMMARY";
include_once __DIR__ . "/../../includes/baseIncludes.php";
include_once __DIR__ . "/../../includes/auth_check.php";

$quarter = "";
$year = "";
$quarter = filter_input(INPUT_GET, 'quarter', FILTER_SANITIZE_SPECIAL_CHARS);
if ($quarter === null || $quarter === false) {
    header("Location: /invoicearchival/list");
    exit();
}
...
<head>
    ...
    <title>GrandMaster ERP | Invoice Archival Summary</title>
                        ...
                        <div class="card-widget-separator-wrapper">
                            <div class="card-body card-widget-separator py-2">
                              <div class="row gy-1 gy-sm-1">
                                <h4 class="my-0">Invoice Archival Summary for <?=getQuarterRange($quarter) . ' ' . $year?></h4>
                              </div>
                            </div>
                        </div>
                    </div>
          ...
          $(function () {
            ...
            if (dt_sparePart_table.length) {
              var dt_spareParts = dt_sparePart_table.DataTable({
                ajax: '/ajax/accounting/fetch_invoice_archival_summary.php',
                columns: [
                ...
                columnDefs: [
                  ...
                  
                  {
                    ...
                    targets: 4,
                    render: function (data, type, full, meta) {
                      var $reference = full['invoiceNo'];
                      var $file = full['file'];
                      if ($file) {
                        return '<a href="' + $file + '" class="btn btn-info" target="_blank">' + $reference + '</a>';
                      } else {
                  ...
                  
                  {
                    ...
                    targets: 6,
                    render: function (data, type, full, meta) {
                      var $invoiceAmount = full['invoiceAmount'];
