<?php
$PAGE_ID = "ACCOUNTS_TRIAL_BALANCE";
...
<head>
    ...
    <title>GrandMaster ERP | Trial Balance</title>
                        ...
                        <div class="card-widget-separator-wrapper">
                            <div class="card-body card-widget-separator py-2">
                                <div class="row gy-1 gy-sm-1">
                                    <h4 class="my-0">Trial Balance</h4>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="datatables-products table">
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td id="footer-debit"></td>   <!-- Debit Total -->
                                        <td id="footer-credit"></td> <!-- Credit Total -->
                                        <td></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
            ...
            $(function () {
                ...
                if (dt_sparePart_table.length) {
                    var dt_spareParts = dt_sparePart_table.DataTable({
                        ajax: '/ajax/accounting/fetch_trial_balance.php',
                        columns: [
                      ...
                      columnDefs: [
                        ...
                        },
                        {
                          ...
                          targets: 6,
                          render: function (data, type, full, meta) {
                            var $outstandingBalance = full['outstandingBalance'];
                            return '<span>' + $outstandingBalance + '</span>';
                          }
                        },
                        {
                          // Balance Type
                          ...
                          render: function (data, type, full, meta) {
                            ...
                            var $balanceType = full['balanceType'];
                            return '<span>' + $balanceType + '</span>';
                          }
                        },
                      ],
                      footerCallback: function (row, data, start, end, display) {
                            let api = this.api();
                            let parseValue = val => parseFloat(val.toString().replace(/,/g, '')) || 0;

                            // Calculate totals across all pages
                            let totalDebit = api.column(4).data().reduce((a, b) => parseValue(a) + parseValue(b), 0);
                            let totalCredit = api.column(5).data().reduce((a, b) => parseValue(a) + parseValue(b), 0);

                            // Update footer cells
                            $(api.column(4).footer()).html(totalDebit.toLocaleString(undefined, { minimumFractionDigits: 2 }));
                            $(api.column(5).footer()).html(totalCredit.toLocaleString(undefined, { minimumFractionDigits: 2 }));
                        },
                        order: [[2, 'asc']],
                        dom:
