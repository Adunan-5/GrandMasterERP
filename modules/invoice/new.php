...
$itemIdsJson = filter_input(INPUT_POST, 'itemIds', FILTER_SANITIZE_FULL_SPECIAL_CHARS);

//$itemIds = json_decode($itemIdsJson, true);
//if (!is_array($itemIds) || empty($itemIds)) {
//    echo json_encode(['status' => "ERROR", 'message' => 'Invalid or empty itemIds']);
//    exit;
//}
...
if ($itemIdsJson !== null) {
    $itemIds = json_decode($itemIdsJson, true);
    if (!is_array($itemIds)) {
        echo json_encode(['status' => "ERROR", 'message' => 'Invalid itemIds format']);
        exit;
    }
}
...
if(isset($_GET['gdnID']) && !empty($_GET['gdnID'])){
    $gdnID = filter_var($_GET['gdnID'], FILTER_SANITIZE_SPECIAL_CHARS);
}
                            ...
                            <div class="card-body card-widget-separator py-2">
                                <div class="row gy-1 gy-sm-1">
                                    <div class="col-6">
                                        ...
                                        $orderID = getOrderManagementIDFromSalesOrderID($refDocID);


                                        $res          = $db->query("SELECT * FROM key_documents WHERE documentId = ?s", $refDocID);
                                        $documentInfo = mysqli_fetch_assoc($res);


                                        $keyDocument = new KeyDocument();
                                        $keyDocument->loadById($refDocID);
                                        ...
                                        if (!empty($itemIds)) {
                                            ...
                                            foreach ($itemIds as $itemId) {
                                                $res = $db->query("SELECT * FROM line_items WHERE documentId = ?s AND itemId = ?s", $refDocID, $itemId);
                                          ...
                                          $pickAndPackres = $db->query("SELECT * FROM pick_and_pack_line_items WHERE orderManagementId = ?s", $orderID);
                                            while($row = mysqli_fetch_assoc($pickAndPackres)) {
                                                // Fetch the original line item details
                                                $originalItemRes = $db->query("SELECT * FROM line_items WHERE documentId = ?s AND itemId = ?s",
                                                    $refDocID, $row['itemId']);
                                                $originalItem = mysqli_fetch_assoc($originalItemRes);

                                                // Merge the pick/pack data with original line item data
                                                $lineItems[] = array_merge($row, [
                                                    'unitPrice' => $originalItem['unitPrice'] ?? 0,
                                                    'vatPercentage' => $originalItem['vatPercentage'] ?? 0,
                                                    'hsCode' => $originalItem['hsCode'] ?? '',
                                                    'hsPercentage' => $originalItem['hsPercentage'] ?? 0,
                                                    'UOM' => $originalItem['UOM'],
                                                    'discountPercentage' => $originalItem['discountPercentage'] ?? 0,
                                                    'invoicedQuantity' => $row['quantity'] ?? 0,
                                        ...
                                        $orderStatus = $documentInfo['orderWHStatus'];

                                        ?>
                                        <h4 class="my-0"><?= getSalesOrderNumberFromDocumentID($refDocID) ?></h4>
                                    </div>
<!--                                    <div class="col-6 text-end">-->
<!--                                        Quotation-->
<!--                                        <a class="btn btn-label-linkedin text-nowrap d-inline-flex position-relative me-4" href="/quotation/preview/--><?php //= $keyDocument->documentId ?><!--">--><?php //= $keyDocument->getQuotationOrderNumberWithPrefix() ?>
<!--                                        </a>-->
<!--                                    </div>-->
                                </div>
                            </div>
                                            ...
                                            <dl class="row mb-0">
                                                ...
                                                <dd class="col-sm-7">
                                                    <div class="input-group input-group-merge disabled">
                                                        ...
                                                        <input name="invoiceNumber" id="invoiceNumber" type="text" class="form-control" disabled placeholder="Will be generated once saved / confirmed" value="NEW"/>
                                                    </div>
                                                ...
                                                <dd class="col-sm-7">
                                                    <input name="invoiceDate" id="invoiceDate" type="text" class="form-control gmm-date-format" placeholder="DD - MMM - YYYY" value="<?php echo date('Y-m-d'); ?>"/>
                                                </dd>
...
<!--                                                </dd>-->
                                                ...
                                                <dd class="col-sm-7">
                                                    <input type="text" class="form-control" readonly id="customerPO" name="customerPO" value="<?= $documentInfo['PONumber'] ?>"/>
                                                    <input type="hidden" class="form-control" readonly id="poAttachmentFileName" name="poAttachmentFileName" value="<?= $documentInfo['POAttachment'] ?>"/>
                                                </dd>
                                                <dt class="col-sm-5 mb-2 d-md-flex align-items-center justify-content-end">
...
<!--                                                              <input name="invoiceQty" type="text" class="form-control numbers-only calculation-trigger" placeholder="Qty" min="1" required value="--><?php //= $item['invoicedQuantity'] ?><!--"/>-->
