...
<head>
  ...
  <title>GrandMaster ERP | PO</title>
</head>
<body>
<!-- Layout wrapper -->
      ...
      <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
          <div class="card mb-6">
            ...
            $rfpDATA = "";
            $res     = $db->query("SELECT * FROM rfp_documents WHERE active = 1 and rfpId = ?s", $rfpID);
            ...
            $warehouseID = $rfpDATA['warehouseId'];
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-5 col-sm-5 col-12 mb-sm-0 mb-6"  style="display: none">
                      <div class="mb-4">
                        ...
                        <select name="destinationWH" id="destinationWH" class="form-select mb-4 w-50 selectpicker w-100" data-style="btn-default" data-live-search="true" tabindex="null" disabled>
                          <option value="">Select Warehouse</option>
                            <?php
                            $res = $db->query("SELECT * FROM warehouses WHERE active ='1'");
                            while ($row = mysqli_fetch_assoc($res)) {
                                ?>
                              <option <?php if ($row['warehouseId'] == $rfpDATA['warehouseId']) echo "selected"; ?> value="<?= $row['warehouseId'] ?>"><?= $row['warehouseName'] . " | " . $row['warehouseCode'] ?></option>
                                <?php
                            }
                            ?>
                        </select>
                ...
                <div class="card-body invoice-preview-header rounded">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-12 mb-4 text-md-end d-flex flex-column align-items-md-end">
                      <div class="svg-illustration mb-4 gap-2 align-items-center">
                        <div>
                          <span class="app-brand-text fw-bold fs-4 ms-50" id="warehouseName"><?= $warehouse->warehouseName ?></span>
                        </div>
                        <div>
                          <span class="app-brand-text fw-bold fs-5 ms-50" id="warehouseCode"><?= $warehouse->warehouseCode ?></span>
                        </div>
                      </div>
                      ...
                      <p class="mb-2" id="warehouseCityPostalCode"><?= getCityFromID($warehouse->cityId) . " " . $warehouse->postalCode ?></p>
                      <p class="mb-2" id="warehouseStateCountry"><?= getStateFromID($warehouse->stateId) . " " . getCountryFromID($warehouse->countryId) ?></p>
                ...
                <div class="card-body pt-0 px-0">
                  ...
                  <form class="source-item">
                    <div class="mb-4" data-repeater-list="line-item">
                        ...
                        <?php
                        $itemCount   = 0;
                        $lineitemres = $db->query("SELECT * FROM rfp_line_items WHERE rfpId = ?s", $rfpID);
...
//                                      while ($row = mysqli_fetch_assoc($res)) {
//                                          ?>
<!--                                        <option --><?php //if($lineitemrow['UOM'] == $row['uomId'])  echo 'SELECTED'?><!-- value="--><?php //= $row['uomId'] ?><!--" data-type="--><?php //= $row['type'] ?><!--" data-ratio="--><?php //= $row['ratio'] ?><!--">--><?php //= $row['uomName'] ?><!--</option>-->
<!--                                          --><?php
//                                      }
//                                      ?>
<!--                                  </select>-->
<!--                                </div>-->
                                ...
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  ...
                                  <input name="hsCode" type="text" class="form-control" placeholder="HS Code" min="1" required value="<?= $lineitemrow['hsCode'] ?>" readonly/>
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  <p class="h6 mb-1">Aisle</p>
                                  <input name="aisle" type="text" class="form-control" placeholder="Aisle" min="1" required value="<?= $lineitemrow['aisle'] ?>" readonly/>
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  <p class="h6 mb-1">Bin</p>
                                  <input name="bin" type="text" class="form-control" placeholder="Bin" min="1" required value="<?= $lineitemrow['bin'] ?>" readonly/>
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  <p class="h6 mb-1">Lot#</p>
                                  <input name="lot" type="text" class="form-control" placeholder="Lot" min="1" required value="<?= $lineitemrow['lotSerial'] ?>" readonly/>
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
  ...
  function savePO() {
    if ($('#supplier').val().length > 0 && $('#destinationWH').val().length > 0) {
      ...
      if (fileInput.files.length > 0) {
        ...
        $.ajax({
          url: '/ajax/podocument/file_upload.php', // Replace with your file upload endpoint
          type: 'POST',
          data: formData,
          processData: false,
          contentType: false,
