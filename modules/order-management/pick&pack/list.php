<?php
$PAGE_ID = "ORDER_MANAGEMENT_PICK_N_PACK_ORDERS_LIST";
...
<head>
    ...
    <title>GrandMaster ERP | Pick n Pack</title>
                        ...
                        <div class="card-widget-separator-wrapper">
                            <div class="card-body card-widget-separator py-2">
                                <div class="row gy-1 gy-sm-1">
                                    <h4 class="my-0">Pick and Pack Orders List </h4>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
  ...
  $(function () {
    ...
    if (dt_invoice_table.length) {
      ...
      let ajaxURL = '/ajax/ordermanagement/fetch_pp_orders.php';
      ...
      var dt_invoice = dt_invoice_table.DataTable({
        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              ...
              return (
                '<div class="d-flex align-items-center">' +
                '<a href="/order-management/picknpack/edit/' + $gdnId + '" data-bs-toggle="tooltip" class="btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Edit"><i class="ti ti-edit mx-2 ti-md"></i></a>' +
                '</div>'
              );
            }
          },

          {
            ...
            targets: 2,
            render: function (data, type, full, meta) {
              var $gdnNumber = full['gdnNumber'];
              var $gdnId = full['gdnId'];

              // Creates full output for row
              // if ($quotationNumber === null) $quotationNumber = "Draft";
              var $row_output = '<a class="text-truncate" href="/order-management/picknpack/edit/' + $gdnId + '"># ' + $gdnNumber + '</a>';
              return $row_output;
            }
          },
          {
...

          {
            ...
            render: function (data, type, full, meta) {
              ...
              if($transactionType === 'SALESORDER') {
                $baseURL = '/salesorders/edit/';
              } else if($transactionType === 'TRANSFER') {
                $baseURL = '/warehouse/transfer/edit/';
              }

              var $row_output = '<a class="text-truncate" href="' + $baseURL + $documentId + '" target="_blank"># ' + $internalRef + '</a>';
              return $row_output;
            }
          },
          {
          ...
          },
          {
            ...
            targets: 8,
            render: function (data, type, full, meta) {
              var $shippingDate = new Date(full['shippingDate']);

              let $row_output;
              if (full['shippingDate'] != null) {
                $row_output = '<span class="d-none">' + moment($shippingDate).format('YYYYMMDD') + '</span><span class="text-truncate">' + moment($shippingDate).format('DD MMM YYYY') + '</span>';
              } else {
                $row_output = "-";
              }
        ...
        language: {
          ...
          search: '',
          searchPlaceholder: 'Search Pick and Pack Order',
          paginate: {
        ...
        responsive: {
          details: {
            display: $.fn.dataTable.Responsive.display.modal({
              header: function (row) {
                var data = row.data();
                return 'Details of ' + data['gdnNumber'];
              }
            }),
            type: 'column',
