            ...
            <div class="card-widget-separator-wrapper">
              <div class="card-body card-widget-separator py-2">
                <div class="row gy-1 gy-sm-1">
                  <div class="col-6">
                    <h4 class="my-0"><?= getRFPNumberFromDocumentID($rfpDATA['rfpId']) ?> | Edit
                      RFP</h4>
                  </div>
                ...
                <div class="card-body invoice-preview-header rounded">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-12 mb-4 text-md-end d-flex flex-column align-items-md-end">
                      ...
                      <p class="mb-2" id="warehouseContact">
                        Contact: <?= $warehouse->phone ?></p>
                    </div>
                  </div>
...
<!--                        </dt>-->
<!--                        <dd class="col-md-8 col-sm-7">-->
<!--                          <input type="text" id="shippingCost" class="form-control due-date"-->
<!--                                 value="--><?php //= $rfpDATA['shippingCost'] ?><!--" />-->
<!--                        </dd>-->
                      </dl>
                    </div>
                  </div>
                            ...
                            <div class="d-flex border rounded position-relative pe-0">
                              <div class="row w-100 p-3">
                                ...
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  ...
                                  <input name="hsCode" type="text" class="form-control" placeholder="HS Code" min="1"
                                         required value="<?= $lineitemrow['hsCode'] ?>" />
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                ...
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  ...
                                  <input name="bin" type="text" class="form-control" placeholder="Bin" min="1"
                                         required value="<?= $lineitemrow['bin'] ?>" />
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                                  <p class="h6 mb-1">Lot#</p>
                                  <input name="lot" type="text" class="form-control" placeholder="Lot" min="1"
                                         required value="<?= $lineitemrow['lotSerial'] ?>" />
                                </div>
                                <div class="col-md-1 col-12 mb-md-0 mb-4">
                        ...
                        <div class="offcanvas-body pt-0 flex-grow-1">
                            ...
                            <div class="mt-5">
                                <ul class="timeline mb-0">
                                    ...
                                    $RFPDocumentHistory = new RFPHistory();
                                    $RFPDocumentHistory->loadByRFPId($rfpID);

                                    if(empty($RFPDocumentHistory->timeline)) {
                                        ?>
                                        <li class="timeline-item timeline-item-transparent">
                                            <div class="timeline-event">
                                                <div class="timeline-header mb-3">
                                                    <h6 class="mb-0">No history available for this RFP</h6>
                                                </div>
                                            </div>
                                        </li>
                                        <?php
                                    ...
                                    foreach ($RFPDocumentHistory->timeline as $historyItem) {
                                        ?>
                                        <li class="timeline-item timeline-item-transparent">
...
<script>
    ...
    $(document).on('change', '#destinationWH', function(e) {
      ...
      $.ajax({
        ...
        success: function(data, status) {
          ...
          $('#warehouseEmail').html('Email: ' + data.email ? data.email : '');
          $('#warehouseContact').html('Contact: ' + data.phone ? data.phone : '');
        },
  ...
  //    // Both file and supplier quotation number are provided, proceed with creating PO
  //    // Get the base URL of the current page
  //    const baseUrl = window.location.origin;
  //    // Define the relative URL to open
  //    const relativeUrl = '/purchaseorder/new?rfpID=' + '<?php //=$rfpID ?>//';
  ...
  function createPO() {
    ...
    if (fileInput && (fileInput.files.length > 0 || fileName) && supplierQuotationNo) {
      ...
      $.ajax({
        url: '/ajax/podocument/check_po_exists.php', // Backend script to check PO existence
        type: 'POST',
        ...
        success: function(response) {
          ...
          if (response.status === 'success') {
            if (response.poExists && response.poId) {
              ...
              const baseUrl = window.location.origin;
              const editUrl = baseUrl + '/purchaseorder/edit/' + response.poId;
              window.open(editUrl, '_blank');
            } else {
              // No PO exists, navigate to the new PO creation page
              const baseUrl = window.location.origin;
              const relativeUrl = '/purchaseorder/new?rfpID=' + '<?=$rfpID ?>';
              const fullUrl = baseUrl + relativeUrl;
              window.open(fullUrl, '_blank');
            }
          } else {
  ...
  function updateRFPStatus(documentID, status, showMessage = true, rejectReason = '') {
    ...
    $.ajax({
      url: '/ajax/rfpdocument/update_rfp_status.php',
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
      success: function(data, status) {
        ...
        if (statusmessage == 'SUCCESS') {
          if (showMessage)
            showSuccessMessage(message, gotoPage, '/rfp/edit/' + documentID);
        }
      },
