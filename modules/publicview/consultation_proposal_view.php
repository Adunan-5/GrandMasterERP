...
$PAGE_ID = "CONSULTATION_PROPOSAL_CUSTOMER_VIEW";
...
<head>
    ...
    <title>GrandMaster ERP | CONSULTATION PROPOSAL</title>
</head>
<body>
<!-- Layout wrapper -->
        ...
        <div class="container-fluid">
            ...
            <div class="content-wrapper">
                ...
                <div class="container-fluid flex-grow-1 container-p-y">
                    ...
                    $resProposal    = $db->query("SELECT * FROM consultation_proposals WHERE proposalID = ?s", $documentID);
                    $rowProposal    = mysqli_fetch_assoc($resProposal);
                    $proposalNumber = getProposalNumberFromProposalID($rowProposal['proposalID']);

                    $customerInfo = new Customer();
                    $customerInfo->loadById($rowProposal['customerID']);
                    ...
                    if (isset($_POST['password'])) {
                        ...
                        if ($rowProposal['customerPassword'] === $password) {
                    ...
                    <div class="row invoice-preview">
                                    ...
                                    <div class="card-body card-widget-separator py-2">
                                        <div class="row gy-1 gy-sm-1">
                                            <div class="col-6">
                                                ...
                                                ?>
                                                <h4 class="my-0"><?= getProposalNumberFromProposalID($documentID) ?> </h4>
                                            </div>
                                            <div class="col-6 text-end">
                                                <?php
                                                switch ($rowProposal['status']) {
                                                    case CONSULTATION_PROPOSAL_STATUS_SENT_TO_CUSTOMER:
                                                        ?>
                                ...
                                <div class="card invoice-preview-card p-sm-12 p-6">
                                    <div class="card-body invoice-preview-header rounded">
                                        <div class="d-flex justify-content-between flex-xl-row flex-md-column flex-sm-row flex-column">
                                            ...
                                            <div>
                                                <h5 class="mb-6">Proposal
                                                    #<?= $proposalNumber ?></h5>
                                                <div class="mb-1 text-heading">
                                                    <span>Date Issued:</span>
                                                    <span><?= formatDate($rowProposal['preparedOn'], false) ?></span>
                                                </div>
                                                <div class="mb-1 text-heading">
                                                    <span>Date Due:</span>
                                                    <span><?= formatDate($rowProposal['validUntil'], false) ?></span>
                                                </div>
                                                <div class="mb-1 text-heading">
                                                ...
                                                <div class="mb-1 text-heading">
                                                    ...
                                                    <span><?= getDisplayNameFromUserID($rowProposal['createdBy']) ?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="row invoice-preview mt-6" id="proposalContainer">
                                    ...
                                    <div class="col-xl-12 col-md-8 col-12 mb-md-0 mb-6">
                                        <div class="card invoice-preview-card p-sm-12 p-6" id="proposalContentContainer">
                                            ...
                                            $baseQuery = "SELECT * FROM consultation_sma WHERE associatedProposalID = ?s";
                                            $resSMAContent = $db->query($baseQuery, $rowProposal['proposalID']);
                                            $rowSMAContent = mysqli_fetch_assoc($resSMAContent);
                                            ?>
                                            <h2>Introduction</h2>
                                            <?= $rowTemplateContent['contentIntroduction'] ?>
                                            <h2>Service & Products</h2>
                                            <div id='proposalServicesListInlineDIV' class='mb-6'>
                                                <div class="table-responsive border border-bottom-0 border-top-0 rounded">
                                                    <table class="table m-0">
                                                        ...
                                                        while($row=mysqli_fetch_assoc($res))
                                                        {
                                                        	?>
                                                            <tr>
                                                                <td class="text-nowrap text-heading"><strong><?=$row['itemName'] ?></strong><br><?=getItemDescriptionForConsultationServiceItemID($row['itemId']) ?></td>
                        ...
                        >
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="alert alert-warning" role="alert">This proposal is password
                                    ...
                                    <div class="card-body">
                                        ...
                                        if ($rowProposal['status'] == CONSULTATION_PROPOSAL_STATUS_CUSTOMER_ACCEPTED) {
                                            ...
                                            <div class="alert alert-primary" role="alert">
                                                <p>You have already accepted this proposal. If you have any queries, please contact us.</p>
                                            </div>
                                            <?php
                                        } elseif ($rowProposal['status'] == CONSULTATION_PROPOSAL_STATUS_CUSTOMER_REJECTED) {
                                            ?>
                                            <div class="alert alert-danger" role="alert">
                                                <p>You have rejected this proposal. If you have any queries, please contact us.</p>
                                            </div>
                                            <?php
                                        } else {
                                            ?>
                                        ...
                                        if ($rowProposal['status'] == "" ) {
                                            ?>
                                            <div class="d-flex mb-1">
                                                <a class="btn btn-label-info d-grid w-100 me-1" target="_blank" href="/quotation/print/<?= encryptString($documentID) ?>">
                                                    <span class="d-flex align-items-center justify-content-center text-nowrap"><i class="ti ti-printer ti-xs me-2"></i>Print</span>
                                                </a>
                                                <a href="/quotation/download/<?= encryptString($documentID) ?>" class="btn btn-label-info d-grid w-100">
...
<script>
    ...
    function acceptProposal() {
        ...
        formData.append('status', '<?=CONSULTATION_PROPOSAL_STATUS_CUSTOMER_ACCEPTED ?>');
        formData.append('rejectReason', "");

        $.ajax({
            url: '/ajax/consultation/customer_update_proposal_status.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function (data, status) {
                ...
                if (statusmessage == "SUCCESS") {
                    Swal.fire({
                        icon: 'success',
                        title: 'Accepted!',
                        text: 'Thank you for accepting the proposal. Have a great day!',
                        customClass: {
                ...
                success: function (data, status) {
                    ...
                    if (statusmessage == "SUCCESS") {
                        Swal.fire({
                            icon: 'error',
                            title: 'Rejected!',
                            text: 'You have rejected the proposal. Please contact us if you need any further clarifications.',
                            customClass: {
                                confirmButton: 'btn btn-success'
                            }
                        }).then(function () {
