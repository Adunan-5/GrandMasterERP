...
class ConsultationKeyDocumentHistory
{
    ...
    public function loadByDocumentId($documentId)
    {
        try {
            ...
            $query = "select * from consultation_key_document_history WHERE keyDocumentId  = ?s ORDER BY historyId DESC";
            $res   = $this->db->query($query, $documentId);

            while ($row = $this->db->fetch($res)) {

                $this->timeline[] = new ConsultationKeyDocumentHistoryItem(documentId: $row['keyDocumentId'],
