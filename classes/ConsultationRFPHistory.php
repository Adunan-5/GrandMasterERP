...
class ConsultationRFPHistory
{
    ...
    public function loadByRFPId($rfpId)
    {
        try {
            ...
            $query = "select * from consultation_rfp_document_history WHERE rfpId  = ?s ORDER BY updatedAt DESC";
            $res   = $this->db->query($query, $rfpId);

            while ($row = $this->db->fetch($res)) {

                $this->timeline[] = new ConsultationRFPHistoryItem(rfpId: $row['rfpId'],
